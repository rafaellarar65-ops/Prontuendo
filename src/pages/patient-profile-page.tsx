  ...
  return <div className="space-y-3">
    ...
    {isLoading ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-slate-400" /></div> : (
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {(data ?? []).map((exam) => { const status = getLabResultStatus(exam); return <tr key={exam.id} className="border-b last:border-none"><td className="py-2 pr-2">{new Date(exam.resultDate).toLocaleDateString('pt-BR')}</td><td className="py-2 pr-2 font-medium text-slate-700">{exam.examName}</td><td className="py-2 pr-2">{exam.value} {exam.unit ?? ''}</td><td className="py-2 pr-2">{exam.reference ?? '—'}</td><td className="py-2"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${status.cls}`}>{status.label}</span></td></tr>;
            })}
          </tbody>
        </table>
      </div>
    ...
    {showModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <form onSubmit={submit} className="w-full max-w-md space-y-3 rounded-2xl bg-white p-5 shadow-2xl">
        ...
        <div className="flex justify-end gap-2"><button type="button" onClick={() => setShowModal(false)} className="rounded-lg border px-3 py-2 text-sm">Cancelar</button><button disabled={isPending} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white">Salvar</button></div>
      </form>
    </div>}
  </div>;
};
...
const GlucoseTab = ({ patientId }: { patientId: string }) => {
  ...
  return <div className="space-y-4">
    ...
    {isLoading ? <div className="flex justify-center py-8"><Loader2 className="animate-spin text-slate-400" /></div> : <ul className="space-y-2">{(data ?? []).slice(0, 20).map((item) => <li key={item.id} className="rounded-xl border border-slate-100 px-3 py-2 text-sm"><span className="font-semibold text-slate-800">{item.value} mg/dL</span><span className="ml-2 text-slate-500">{new Date(item.measuredAt).toLocaleString('pt-BR')}</span>{item.notes && <p className="text-xs text-slate-500">Obs: {item.notes}</p>}</li>)}</ul>}
    {showModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><form className="w-full max-w-md space-y-3 rounded-2xl bg-white p-5 shadow-2xl" onSubmit={(e) => { e.preventDefault(); mutate({ patientId, value: Number(value), measuredAt: new Date().toISOString(), ...(notes ? { notes } : {}) }, { onSuccess: () => setShowModal(false) }); }}><h3 className="font-semibold text-slate-800">Registrar glicemia</h3><input required type="number" placeholder="mg/dL" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" value={value} onChange={(e) => setValue(e.target.value)} /><textarea placeholder="Observação (opcional)" className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" value={notes} onChange={(e) => setNotes(e.target.value)} /><div className="flex justify-end gap-2"><button type="button" onClick={() => setShowModal(false)} className="rounded-lg border px-3 py-2 text-sm">Cancelar</button><button disabled={isPending} className="rounded-lg bg-indigo-600 px-3 py-2 text-sm font-medium text-white">Salvar</button></div></form></div>}
  </div>;
};
...
const BioimpedanceTab = ({ patientId }: { patientId: string }) => {
  ...
  return <div className="space-y-3">
    ...
    <ul className="space-y-2">
      {(data ?? []).slice().reverse().map((item) => <li key={item.date} className="rounded-xl border border-slate-100 px-3 py-2 text-sm text-slate-700">{new Date(item.date).toLocaleDateString('pt-BR')} · Gordura {item.fatMassPercent}% · Massa muscular {item.muscleMassKg} kg</li>)}
    </ul>
  </div>;
};
