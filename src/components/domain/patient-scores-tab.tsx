...
export const PatientScoresTab = ({ patientId, patient }: { patientId: string; patient: Patient }) => {
  const [activeScore, setActiveScore] = useState<CalculatorId>('homa_ir');
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [form, setForm] = useState<Record<string, string>>({});

  const { data: allHistory } = useScoreHistoryQuery(patientId);
  const { data: activeHistory } = useScoreHistoryQuery(patientId, activeScore as ScoreType);
  const {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="space-y-3 rounded-lg border bg-white p-4">
        ...
        {chartData.length === 0 ? <p className="text-sm text-slate-500">Sem registros para este escore.</p> : null}
